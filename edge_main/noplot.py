    ...
    while not shutdown_event.is_set():
        ...
        if is_scanning:
            try:
                ...
                # Server sẽ vẽ, client chỉ gửi frame gốc
                annotated = frame
                ...
                if result.boxes is not None and len(result.boxes) > 0:
                    ...
                    for i in range(len(boxes)):
                        ...
                        # Thu thập dữ liệu detection để gửi đi
                        detections.append({
                            "box": boxes[i].tolist(),
                            "label": label,
                            "conf": float(confs[i])
        ...
        # ===== SEND TO QUEUE =====
        try:
            sender_frame_queue.put_nowait((annotated, frame_counter, detections, curr_time))
        except queue.Full:
            try:
...
if __name__ == "__main__":
    ...
    try:
        ...
        while not shutdown_event.is_set():
            try:
                frame, counter, detections, timestamp = sender_frame_queue.get(timeout=1.0)
            except queue.Empty:
                continue
